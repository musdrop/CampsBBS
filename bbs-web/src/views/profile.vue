<template>
  ...
  <h3 style="color:gray">我的信息</h3>
  <div>
    <el-card shadow="hover" class="info-card">
      <div style="text-align: left">
        <p>用户名：{{loginUser.name}}</p>
        <p>账号：{{loginUser.account}}</p>
        <p>邮箱：{{loginUser.mail}}</p>
      </div>
    </el-card>
  </div>
  <h3 style="color:gray">我发布的帖子</h3>
  <div>
    <el-scrollbar height="50vh">
      <!--帖子每行-->
      <el-row
        :gutter="24"
        v-for="list in forumList"
        :key="list"
        style="margin-top: 2%"
      >
        <!--帖子行中的每列-->
        <el-col :span="6" v-for="forum in list" :key="forum">
          ...
          <el-card shadow="hover" class="box-card" @click="handleDetail(forum)">
            ...
            <div style="padding: 0px">
              ...
              <!--底部帖子发布时间-->
              <div class="bottom">
                <time class="time" style="font-size: 16px; color: #afa9a9">{{
                  forum.createTime
                }}</time>
              </div>
            </div>
